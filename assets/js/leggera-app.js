...
function mixWrapper() {
    ...
    const leggeraMethods = {
            ...
            changeLeggeraTheme: function() {
                if (Number(lightTheme) === 0) { lightTheme = 1; } else { lightTheme = 0; }
                ...
                if (leggeraVariables.botaoVistaColap.classList.contains('btn-info')) {
                    ...
                    for (i = 1; i <= leggeraVariables.colapList.length; i++) {
                        ...
                        if (saveBtn.classList.contains('no-display') == false) {
                            alert(`Não é possível aceder à textarea principal, enquanto existirem alterações pendentes.`);
                            return
                        }
                    }
                }
    ...
    const leggeraManuais = {
                ...
                $.ajax({
                    ...
                    success: function(rsp) {
                        ...
                        for (i = 0; i < rsp.length; i++) {
                            ...
                            try { leggeraManuais.manuaisUtilizador[i] = (rsp[i].code.toString()); } catch {}
                        }
                    },
                    error: function(rsp) {
                        // Modal com os manuais recebidos de BD
                ...
                switch (action) {
                    case 'new':
                        ...
                        $.ajax({
                            ...
                            success: function(rsp) {
                                if (rsp.startsWith('Err')) { console.log('fudeu') } else if (rsp.includes('atual')) {
                    ...
                    case 'delete':
                        $.ajax({
                            ...
                            success: function(rsp) {
                                if (rsp.startsWith('Err')) { console.log('fudeu') } else {
            ...
            saveManual: function(e) {
                ...
                if (e.target.tagName === "I") {
                    manualName = e.target.parentElement.parentElement.firstChild.innerText;
                    action = 'update';
                };
    ...
    const leggeraTextboxes = {
            ...
            displayControls: function() {
                ...
                for (i = 1; i <= leggeraGOAssets.textboxesFromDB.length; i++) {
                    if (i >= 4) { textboxControls.appendChild(leggeraMethods.mambo('div', id = `textbox-${i}`, cla = 'col-sm-6 helpcenter-textbox', inn = leggeraGOAssets.textboxesFromDB[i - 1])); } else { textboxControls.appendChild(leggeraMethods.mambo('div', id = `textbox-${i}`, cla = 'col-sm-4 helpcenter-textbox', inn = leggeraGOAssets.textboxesFromDB[i - 1])); }
    ...
    const leggeraIcons = {
            ...
            writeIcon: function(e) {
                ...
                if (lightTheme == 1) { icon.style.color = '#fff' } else { icon.style.color = '#000' };
            }
        }
        // ################ botões & chips
    const leggeraButtons = {
        ...
        changeCurrentTheme: function(e) {
            ...
            switch (eTarget.id) {
                case 'theme-1':
                    leggeraButtons.displayControls(e, 1);
                    break
                case 'theme-2':
                    leggeraButtons.displayControls(e, 2);
                    break
                case 'theme-3':
                    leggeraButtons.displayControls(e, 3);
                    break
                case 'theme-4':
                    leggeraButtons.displayControls(e, 4);
            }
        },
    ...
    const leggeraListsAndTables = {
            ...
            writeTable: function() {
                ...
                switch (leggeraVariables.tableType) {
                    case 'normal-table':
                        novaTabela = leggeraMethods.mambo('table', id = leggeraVariables.tableID, cla = 'phcgo-old-table');
                        break
                    case 'modern-table':
                        novaTabela = leggeraMethods.mambo('table', id = leggeraVariables.tableID, cla = 'phcgo-new-table');
                        break
                    case 'modern-table-blue':
                        novaTabela = leggeraMethods.mambo('table', id = leggeraVariables.tableID, cla = 'phcgo-new-table-blue');
                        break
                }
                ...
                function tableWidthLegoo(cla) {
                    ...
                    if (tablewidth === null || tablewidth === '') { return } else {
            ...
            convertTableEngine: function() {
                ...
                for (i = 0; i <= numLinhas; i++) {
                    ...
                    for (x = 0; x < itemsParaConverter[i].length; x++) {
                        ...
                        try { tabelaConvertida.lastChild.lastChild.setAttribute('colspan', colspan.value) } catch {}
                        let rowspan = itemsParaConverter[i][x].attributes.rowspan;
                        try { tabelaConvertida.lastChild.lastChild.setAttribute('rowspan', rowspan.value) } catch {}
                    }
                }
            ...
            toogleColapsablesappControls: function() {
                ...
                if (leggeraVariables.colapHCPreview.classList.contains('no-display')) {
                    ...
                    for (i = 1; i <= leggeraVariables.colapList.length; i++) {
                        let saveBtn = document.querySelector(`#colap-save-btn-${i}`);
                        if (saveBtn.classList.contains('no-display') == false) {
                            alert(`Não é possível retornar à vista principal, enquanto existirem alterações pendentes.`);
                            return
                        }
                    }
            ...
            novoColapsavel: function() {
                if (leggeraVariables.colapList.length === 0) {
                    ...
                    for (i = 1; i <= leggeraVariables.colapList.length; i++) {
                        let saveBtn = document.querySelector(`#colap-save-btn-${i}`);
                        if (saveBtn.classList.contains('no-display') == false) {
                            alert(`Não é possível adicionar um novo colapsável, enquanto existirem alterações pendentes.`);
                            return
                        }
                    }
    ...
    const leggeraListeners = {
        allEvents: function() {
            ...
            document.querySelector('#userstats-btn').addEventListener('click', function() {
                leggeraVariables.hcPreview.innerHTML = '';
                leggeraMethods.displayUserStats();
            });
