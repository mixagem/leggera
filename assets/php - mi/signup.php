...
$svurl = 'http://mambosinfinitos.pt/leggera/v1-28';
...
$mail->isHTML(true);
// Mail subject 
$mail->Subject = 'SUPERLEGGERA - Criação da conta "' . $un . '"';
...
if ($temuserigual == 0) {
    ...
    $query = "INSERT INTO users (id, username, password, name, email, unlocktoken, image) 
    VALUES ('$totalusers'+1, '$un', '$newpw', '$nome', '$email', '$token', '$image')";
    $result = mysqli_query($con, $query);
    if (mysqli_affected_rows($con) == 1) {
        ...
        <h1 style='font-family:Segoe UI, Tahoma, Geneva, Verdana, sans-serif;padding-left:40px;padding-top:40px;'>Olá
            " . $nome . ",</h1>
        <p style='font-family:Segoe UI, Tahoma, Geneva, Verdana, sans-serif;padding-left:20px'>A tua conta <span style='font-weight: bold;color:rgb(56,92,255)'>SUPERLEGGERA</span> foi criada com sucesso!<br>
        ...
        <div style='display:flex;justify-content:center;align-self:center;padding:20px 0px;'>
            <a style='border:1px solid black;padding:5px 20px;font-size:20px;color:rgb(56,92,255);font-weight:bold;font-family:Segoe UI, Tahoma, Geneva, Verdana, sans-serif;border-radius: 5px;' href='" . $svurl . "/assets/php/activate.php?t=" . $token . "&u=" . $un . "&n=" . $nome . "'>" . $svurl . "/assets/php/activate.php?t=" . $token . "&u=" . $un . "&n=" . $nome . "</a>
        </div>
        ...
        $mail->AltBody = "Olá " . $nome . "\nA tua conta SUPERLEGGERA foi criada com sucesso!\nPara validares o teu endereço de e-mail e ativares a tua conta, visita a ligação seguinte:\n\n>" . $svurl . "/assets/php/activate.php?t=" . $token . "&u=" . $un . "&n=" . $nome;
        if (!$mail->send()) {
        ...
        echo "Erro: Ocorreu um erro ao criar o utilizador.";
    }
} else {
